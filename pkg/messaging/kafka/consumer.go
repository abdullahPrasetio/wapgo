...
// Transient fetch errors are retried with exponential backoff (1 s → 30 s).
...
func (c *Consumer) Start(ctx context.Context, handler HandlerFunc) error {
	...
	for {
		...
		if err != nil {
			...
			c.log.Error().Err(err).Dur("retry_in", bo).Msg("kafka fetch message failed")
			select {
			case <-ctx.Done():
				return nil
			case <-time.After(bo):
		...
		bo = time.Second // reset on successful fetch
